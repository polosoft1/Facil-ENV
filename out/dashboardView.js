...
class DashboardViewProvider {
    context;
    manager;
    static viewId = 'easyEnvDashboard';
    currentView;
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            ...
            switch (msg.command) {
                ...
                case 'openFolder': {
                    const env = msg.envName ? await this.pickEnvByName(msg.envName) : undefined;
                    if (env)
                        vscode.commands.executeCommand('easyenv.openEnvFolder', env);
                    break;
                }
                case 'openTerminal': {
                    const env = msg.envName ? await this.pickEnvByName(msg.envName) : undefined;
                    if (env)
                        vscode.commands.executeCommand('easyenv.openEnvTerminal', env);
                    break;
                }
                case 'activate': {
                    const env = msg.envName ? await this.pickEnvByName(msg.envName) : undefined;
                    if (env)
                        vscode.commands.executeCommand('easyenv.activateEnv', env);
                    break;
                }
                case 'refresh': {
                    await this.updateHtml(webviewView);
    ...
    // =============== Helpers ===============
    async pickEnvByName(name) {
        const envs = await this.manager.scanEnvs();
        return envs.find(e => e.name === name);
    }
    async updateHtml(webviewView) {
        // Leer configuración de Python
        const pythonConfig = vscode.workspace.getConfiguration('python');
        const autoActivate = pythonConfig.get('terminal.activateEnvironment') ?? true;
